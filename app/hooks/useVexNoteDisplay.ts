...
export function useVexNoteDisplay(
  ...
  useEffect(() => {
    if (!rendererRef.current || !note || octave === null) return;
    ...
    try {
      // Convert note name to VexFlow format (e.g., "C#" -> "c#/4", "A" -> "a/4")
  ...
  }, [rendererRef, canvasRef, note, octave]);
}
